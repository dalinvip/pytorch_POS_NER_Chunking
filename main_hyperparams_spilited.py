...
def load_Conll2000(args):
    ...
    # read file
    data_loader = DataConll_Loader_spilited.DataLoader()
    ...
    #                                                    args=args)
    train_iter, test_iter = create_iter.createIterator(batch_size=[args.batch_size, args.batch_size],
                                                       data=[train_data, test_data], operator=create_alphabet,
                                                       args=args)
...
# load data / create alphabet / create iterator
def load_Conll2003(args):
    print("Loading Conll2003 Data......")
    # read file
    data_loader = DataConll_Loader_spilited.DataLoader()
...
def main():
    ...
    if args.Conll2000 is True:
        train_iter, test_iter, create_alphabet = load_Conll2000(args)
    if args.Conll2003 is True:
        train_iter, dev_iter, test_iter, create_alphabet = load_Conll2003(args)
    ...
    model = None
    if args.model_PNC is True and args.ininital_from_Pretrained is False:
        print("loading PNC(POS,NER,Chunking) model.....")
        model = PNC(args)
        shutil.copy("./models/model_PNC.py", args.save_dir)
    else:
        print("loading PNC(POS,NER,Chunking) model from pretrained.")
        model = model_PNC_Pretrained.PNC(args)
        shutil.copy("./models/model_PNC_Pretrained.py", args.save_dir)
    print(model)
